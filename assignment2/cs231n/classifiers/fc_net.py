...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1):
        ...
        preX = res
        if self.use_dropout:
            preX, drop_cache[i] = dropout_forward(preX, self.dropout_param)

    l = self.num_layers
    scores, lastCache = affine_forward(preX, self.params['W%d' % l], self.params['b%d' % l])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    loss, grads = 0.0, {}
    loss, dout = softmax_loss(scores, y)
    loss += 0.5 * self.reg *(sum([np.sum(self.params['W%d' % (i+1)] * self.params['W%d' % (i+1)]) for i in range(self.num_layers)]))
    ############################################################################
    ...
    for i in range(self.num_layers - 1)[::-1]:
        if self.use_dropout:
            preG = dropout_backward(preG, drop_cache[i])
        if self.use_batchnorm:
